<template>
    <div :class="'media d-flex align-items-start'" v-if="message && message.id">
        ...
        <div :class="'preview flex-shrink-0' + (isMessageFileImage() ? ' preview-image' : '')" v-if="getMessageFile()">
            ...
            <div class="msg-box">
                <div class="message-sub-box">
                    <h4 class="d-flex justify-content-between">
                        <strong>
                            {{ getMessageSender() ? getMessageSender().user.name : '' }}
                        </strong>
                    ...
                    <p v-html="getMessageFile() ? getMessageFile().name : (message.message ? message.message : '')"></p>
...
export default {
    name: 'ChatTalkFooterMessageReply',
    components: {
    ...
    computed: {
        getMessageSender() {
            return () => {
                return this.chat.participants.find((participant) => participant.userId === this.message.userId);
            }
        },
        getMessageFile() {
            return () => {
                return this.message && this.message.files && this.message.files.length ? this.message.files[0] : null;
            }
        },
