...
@foreach($properties as $propertyIndex => $property)
    <div class="stage-fields-row"
         data-prop="{{ 'stages[' . $stage->id . '][fields][' . $property . ']' }}"
         style="padding: 15px 0;"
    >
        @include('pages.order.form.stage.fields.' . ($usePropertyIndexInFile ? $propertyIndex . '-' : '') . $property, [
            'title' => ($usePropertyIndexInFile ? $propertyIndex . '. ' : '') . __('stages.' . str_replace('-', '_', $property)),
            'titleClass' => 'col2 col-md-4 col-lg-4 col-xl-4',
            'leftColumnClass' => '',
            'rightColumnClass' => '',
            'prefixForId' => 'stages-' . $stage->id . '-fields-' . $property . '-',
            'prefixForName' => 'stages[' . $stage->id . '][fields][' . str_replace('-', '_', $property) . ']',
        ])
    </div>
@endforeach
